...
export function dispatch(name, args, data) {
  ...
  switch (name) {
    ...
    case "customers": { if (args.merchant) { const m = (META.merchant_proof_index || []).find(p => p.merchant.toLowerCase().includes(args.merchant.toLowerCase())); return m || { error: "not found", all: (META.merchant_proof_index || []).map(p => p.merchant) }; } return { merchant_proof: META.merchant_proof_index, verdict: "Shopify's agentic DEMOS use mock products + a fake buyer persona ('Stella Bandoc') — no merchant shown transacting live via an agent. The only real-merchant proof is two unsourced revenue stats (Omnilux 3.2%, Cozy Earth 20x YoY), no methodology. Go verify the named stores." }; }
